...
public class ChangePINScreen extends JFrame {
    public ChangePINScreen(User user) {
        Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();

        setTitle("Ubah PIN");
        setSize(screenSize.width, screenSize.height); 
        setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE); // Menutup hanya halaman ini
        setLayout(null);
            ...
            if (!(oldPin.isEmpty() || newPin.isEmpty() || newPin2.isEmpty())) {
                if(Integer.parseInt(oldPin) == Integer.parseInt(user.getPassword())) {
                    if(newPin.equals(newPin2)) {
                        ...
                        JOptionPane.showMessageDialog(ChangePINScreen.this, "Ubah PIN berhasil!");

                        ChangePINScreen.this.dispose();
                    } else {
                        JOptionPane.showMessageDialog(ChangePINScreen.this, "Periksa PIN Baru Kembali!", "Error", JOptionPane.ERROR_MESSAGE);
                    }
                } else {
                    JOptionPane.showMessageDialog(ChangePINScreen.this, "PIN lama salah!", "Error", JOptionPane.ERROR_MESSAGE);
                }
            }
            });
